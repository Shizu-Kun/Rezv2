...
function TelegraPh (Path) {
	return new Promise (async (resolve, reject) => {
		if (!fs.existsSync(Path)) return reject(new Error("*FILE TIDAK ADA/EXPIRED*"))
		try {
			...
			return resolve("*PROCESS SUCCESS* ✅\n*Url :* https://telegra.ph" + data.data[0].src + '\n*ExpiredDate :* _NoExpired_')
		} catch (err) {
	...
	return new Promise((resolve, reject) => {
		 ...
		 axios({
			  ...
			  axios({
				   ...
				   resolve({
						status: true,
						message: "Created By Shannz-Xbot",
						result: result
				   })
			  }).catch(reject)
		 }).catch(reject)
