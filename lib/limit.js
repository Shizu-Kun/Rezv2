...
exports.isLimit = function(sender, isPremium, isOwner, limitCount, _db){
    ...
    if (found === false) {
        ...
        _db.push(obj)
        fs.writeFileSync('./setup/database/limit.json', JSON.stringify(_db, null, 2))
        return false
    }
}
exports.limitAdd = function(sender, _db){
	...
	Object.keys(_db).forEach((i) => {
		...
		if (found !== false) {
			_db[found].limit += 1
			fs.writeFileSync('./setup/database/limit.json', JSON.stringify(_db, 2))
		}
}
...
exports.giveLimit = function(pemain, duit, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./setup/database.json', JSON.stringify(_db, null, 2))
    } else {
...
exports.addBalance = function(sender, duit, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./setup/database/balance.json', JSON.stringify(_db, null, 2))
    } else {
        const bulin = ({
            id: sender,
            balance: duit
...
exports.isGame = function(sender, isOwner, gcount, _db){
    ...
    if (found === false){
        ...
        _db.push(obj);
        fs.writeFileSync('./setup/database/glimit.json',JSON.stringify(_db, null, 2));
        return false;
    }
}
...
exports.createHit = function(sender, _db){
    ...
    _db.push(anohoh);
    fs.writeFileSync('./setup/database/userhit.json',JSON.stringify(_db, null, 2));''
